...
def metrics(fn):
    ...
    lo = [i for i in range(41) if H[i][90] >= H_MIN]
    ground = max(abs(P[0][j]) for j in range(181))
    inter  = max(abs(P[i][j]) for i in lo for j in range(181))
    usurf  = max(abs(U[0][j]) for j in range(181))
