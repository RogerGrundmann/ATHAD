...
*
* ATHAD: THE DENSITY IS INSIDE THE INTEGRAL AND INSIDE THE ZONAL MEAN. What was here was
*
*     const double rho = r_air;               // Boussinesq reference density
*     coeff = two_pi * a * cosphi * rho;      // ... applied at EVERY level
*
* i.e. one constant, the SURFACE density, used from the ground to the lid. That is a volume
* flux with a kg/s label. On Earth's 16 km shell rho spans about 6x and the error is a
* stretch factor; here it spans FOUR ORDERS OF MAGNITUDE (43 kg/m3 at the sea, 1e-3 at
* 300 km), so a cell at the lid was weighted ~40 000x too heavily and the thick upper
* layers -- 23 km each at im = 41 -- dominated an integral that should be dominated by the
* bottom few kilometres. The diagnostic could not have shown a Hadley cell here at any
* strength. Same defect class as the rest: an Earth constant in a kernel, latent at 1 bar.
*
...
void cAtmosphereModel::write_meridional_streamfunction(int iter){
    ...
    // both over fluid cells. r_humid is dimensional [kg/m³] (ThermoAtm builds it as
    // 1e2*p/(R_loc*T*water_factor)), so rho*v*u_0 is already a mass flux.
    ...
    // the whole cell-structure argument resting on the scalar Psi_max below. See the Psi
    // declaration in cAtmosphereModel.h for why that mattered.
    for(int i = 0; i < im; i++)
        for(int j = 0; j < jm; j++)
            for(int k = 0; k < km; k++)
                Psi.x[i][j][k] = psi[i][j];
