...
void cAtmosphereModel::RHS_Atmosphere_Turb(int i, int j, int k, const CellGeometry& geo){
    ...
    // Mirror of the vbud_*/wbud_* blocks below, for the VERTICAL wind. This is the component
    // that had no instrument: item 28's spurious 293-rms radial acceleration was invisible
    // because nothing decomposed rhs_u. Note what the split makes checkable at a glance —
    // with the default switches coriolis_nontraditional() is false, so ubud_cor should be
    // identically zero, and metric_curvature() is false too; if either is nonzero, a term the
    // README assumes inert is live. ubud_buoy isolates item 34's extra-*dt suspect.
    if(ubudget_capture){
