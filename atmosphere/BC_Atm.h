...
*/
    ...
    void bcRadius()
    {
        ...
        // Pattern A: ZERO-GRADIENT COPY at both i=0 and i=im-1 (was cubic at both — see the
        // note at the loop itself, README items 52-53).
...
//        Array* cubic_bot_vn_top[] = { &m.c, &m.cloud, &m.ice, &m.gr, &m.co2,
        ...
        for (int j = 0; j < m.jm; j++) {
            for (int k = 0; k < m.km; k++) {
                ...
                //
                // The lid used to be the same cubic. It is the third time this file has had to
                // remove that stencil from a boundary: v/w "overshoot THROUGH zero", the
                // turbulence scalars "amplify the concavity", p_stat reached -36 hPa (Pattern D
                // below). Here it was putting NEGATIVE values into fields that cannot be
                // negative — README item 52 measured s_u = -3.86, s_d = -5.85 and q_v_u = -1459
                // g/kg at exactly 300 005 m, i.e. a NEGATIVE ABSOLUTE TEMPERATURE, and min s =
                // 0.575 (77 K) where the isothermal skin is 263 K.
                //
                // Not one of these fields wants curvature projected past the boundary: they are
                // convection scalars, microphysical rates, mass fluxes and diagnostic forces,
                // all of which decay toward the lid. A zero-gradient copy is non-amplifying and
                // cannot manufacture a sign. The lid values are read — rhsForcing's flux
                // divergence at i = im-2 uses M_u/M_d/s/s_u/s_d/q_v_u at i = im-1 — so this is
                // not only cosmetic.
