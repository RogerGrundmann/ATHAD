...
private:
    ...
    // 18.9x at 256.0 km, all 181 columns, RH 2518 % and 626 %). Print-only: it writes no field
    ...
    // Reject a condensation step whose own latent heating leaves the cell SUPERHEATED, i.e.
    // in a state IceSchemeCommon::canCondense forbids and ThreeCatIceScheme then undoes.
    // README item 75. DEFAULT ON; ATM_SAT_SUPERHEAT=0 restores the old behaviour.
    // item 75 liveness counters — is the guarded write-back even reached?
