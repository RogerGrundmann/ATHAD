...
namespace IceSchemeCommon {
    ...
    inline void evaporateWhereImpossible(cAtmosphereModel& m, double t_u, int i, int j, int k) {
        ...
        if (q_cond > 0.0) {
            ...
            // Item 57: with the carrier renormalised, q_v + q_CO2 + q_bg == 1 for ANY
            // q_v in [0,1] — the carrier simply shrinks as water grows — so the old
            // c <= 1 - co2 ceiling has no meaning under that convention and 1.0 is the
            // real bound on a mass fraction. (That ceiling had already stopped firing,
            // README item 23.) The legacy form is kept when the knob is off.
